...
# def sumarize_and_provide_links():

# In progress...
@mcp.tool()
def research_tool(prompt: str) -> str:
    search_queries = generate_search_queries(prompt)
    search_queries_and_links = get_search_query_links(search_queries)
    markdown_results = convert_html_to_markdown(search_queries_and_links)

    print(markdown_results)

"""
Then, add another function here that uses the LLM to strip out the unneeded text for each result...
